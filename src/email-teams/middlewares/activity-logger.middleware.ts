...
export class ActivityLoggerMiddleware {
  ...
  use(req: Request, res: Response, next) {
    ...
    if (method.toLocaleLowerCase() == 'delete') {
      message = `Delete email team`;
      logType = LogType.DELETE;
    } else if (method.toLocaleLowerCase() == 'post') {
      message = `Create email team`;
      logType = LogType.CREATE;
    } else {
      message = `Update email team`;
    ...
    const infoActivity = {
      ...
      method,
      logType,
      hashTag: HashTag.EMAIL_TEAM,
    };
    this.logActivityServices.addEmailTeamActivity(infoActivity);
    next();
  }
}
